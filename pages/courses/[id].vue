...
async function fetchCourseData() {
  try {
    ...
    if (directResult) {
      course.value = {
        ...directResult,
        stats: directResult.stats ||
          directResult.metadata?.stats || { enrolled: 0, completed: 0 },
        community: directResult.community ||
          directResult.metadata?.community || {
            discussions: 0,
            activeStudents: 0,
...
const currentLesson = computed(() => {
  ...
  if (
    ...
    !course.value.modules[0]?.lessons
  )
    return null
...
const getPrivateClassPrice = () => {
  if (
    !course.value ||
    !course.value.instructor?.availableService?.offers?.[0]
  ) {
    return 'Contact for pricing'
  }
...
const communityData = computed(
  () =>
    (course.value as any)?.metadata?.community || {
      discussions: 0,
      activeStudents: 0,
      nextLiveQ_A: new Date().toISOString(),
    }
...
const statsData = computed(
  () =>
    (course.value as any)?.metadata?.stats || {
      enrolled: 0,
      completed: 0,
    }
)
          ...
          <div class="lg:col-span-2 space-y-8">
            ...
            >
              <div class="aspect-video">
                <mux-player
                  v-if="
                    currentLesson.videoProvider === 'mux' ||
                    currentLesson.video?.playbackId
                  "
                  stream-type="on-demand"
                  :playback-id="
                    currentLesson.video?.playbackId || currentLesson.videoId
                  "
              ...
              <div class="p-4">
                ...
                <p class="text-sm text-muted-foreground">
                  Duration:
                  {{
                    currentLesson.timeRequired || currentLesson.videoDuration
                  }}
                </p>
              </div>
            </div>
            ...
            <div
              class="bg-background rounded-xl shadow-sm overflow-hidden"
              v-if="course.learningResources && course.learningResources.length"
            >
              <div class="p-4 border-b">
                  ...
                  <div>
                    ...
                    <div class="space-y-2">
                      <p
                        ...
                        v-if="course.instructor.knowsLanguage"
                      >
                      ...
                      <div
                        class="text-sm text-muted-foreground"
                        v-if="course.instructor.experience?.achievements"
                      >
