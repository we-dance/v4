          ...
          <div>
            <div class="text-sm text-muted-foreground mb-1">
              {{ sub?.status === 'active' ? 'Active subscription' : 
                sub?.status === 'canceled' ? 'Canceled subscription' : 'Subscription' }}
            </div>
            <div class="flex items-center gap-2">
              <h3 class="text-base font-medium">{{ sub?.name }}</h3>
              <Badge v-if="sub?.status === 'active'" variant="default" class="bg-green-500">Active</Badge>
              <Badge v-else-if="sub?.status === 'canceled'" variant="destructive">Canceled</Badge>
              <Badge v-else-if="sub?.status === 'past_due'" variant="default" class="bg-yellow-500">Past Due</Badge>
            </div>
            <div class="text-sm space-y-1 mt-2">
              <p class="text-foreground">
                {{ sub?.plan }} plan • {{ sub?.price }}
          ...
          >
            ...
            <div class="flex-1 w-full p-6 space-y-4 overflow-auto">
              <p class="text-base text-foreground font-medium mb-2">
                Are you sure you want to cancel your subscription to "{{ subscriptionToCancel?.name }}"?
              </p>
              ...
              <div class="text-sm text-muted-foreground space-y-2 mb-6">
                ...
                <p v-if="subscriptionToCancel?.nextBillingDate" class="font-medium">
                  Subscription will end on: {{ formatDate(subscriptionToCancel.nextBillingDate) }}
                </p>
              </div>
